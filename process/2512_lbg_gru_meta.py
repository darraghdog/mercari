...
# Parse our emojis
print('[{}] Pull the emojis to a new column...'.format(time.time() - start_time))

MAX_EMOJIS = 5
train["name_emoji"] = [' '.join(list(emoji_pattern.sub(r'', sent))[:MAX_EMOJIS]) for sent in train["name"].values]
test ["name_emoji"] = [' '.join(list(emoji_pattern.sub(r'', sent))[:MAX_EMOJIS]) for sent in test["name"].values]
train["name_emoji"] = [l if l != '' else 'blank' for l in train["name_emoji"]]
test["name_emoji"] = [l if l != '' else 'blank' for l in test["name_emoji"]]
# train.groupby('name_emoji')['price'].agg(['mean', 'count']).sort_values('count').tail(100)
...
def tokenize_pool(sent, cutoff = False):
    return " ".join(toktok.tokenize(sent)) 
def tokenize_pool_cut(sent, cutoff = False):
    if cutoff:
        sent = sent[:DESCRIPTION_CUTOFF]
    return " ".join(toktok.tokenize(sent)) 
pool = mp.Pool(processes=4)
train['name_token']             = pool.map(tokenize_pool, train['name'].str.lower().tolist())
test ['name_token']             = pool.map(tokenize_pool, test ['name'].str.lower().tolist())
train['item_description_token'] = pool.map(tokenize_pool_cut, train['item_description'].str.lower().tolist())
test ['item_description_token'] = pool.map(tokenize_pool_cut, test ['item_description'].str.lower().tolist())
...
@jit
def myTokenizerFitJit(strls, max_words, filt):
    list_=[]
    for sent in strls:
        if filt:
            sent = rgx.sub('', sent)
...
def myTokenizerFit(strls, max_words, filt = True):
    mc = myTokenizerFitJit(strls, max_words, filt)
...
# chk = myTokenizerFitJit(train.name_emoji.str.lower(), max_words = 200, filt = False)

tok_raw_cat = myTokenizerFit(train.category_name_split.str.lower(), max_words = 1200)
#tok_raw_nam = myTokenizerFit(train.name.str.lower(), max_words = 25000)
tok_raw_dsc = myTokenizerFit(train.item_description_token.str.lower(), max_words = 40000)
tok_raw_ntk = myTokenizerFit(train.name_token.str.lower(), max_words = 25000)#, filt = False)
tok_raw_emj = myTokenizerFit(train.name_emoji.str.lower(), max_words = 200)#, filt = False)

'''
tok_raw_cat = myTokenizerFit(train.category_name_split.str.lower(), max_words = 500)
#tok_raw_nam = myTokenizerFit(train.name.str.lower(), max_words = 25000)
tok_raw_dsc = myTokenizerFit(train.item_description_token.str.lower(), max_words = 10000)
tok_raw_ntk = myTokenizerFit(train.name_token.str.lower(), max_words = 10000, filt = False)
tok_raw_emj = myTokenizerFit(train.name_emoji.str.lower(), max_words = 100, filt = False)
'''
...
train["seq_item_description"] =        fit_sequence(train.item_description_token.str.lower(), tok_raw_dsc)
test["seq_item_description"] =         fit_sequence(test.item_description_token.str.lower(), tok_raw_dsc)
...
train["seq_name_emoji"] =              fit_sequence(train.name_emoji.str.lower(), tok_raw_emj)#, filt = False)
test["seq_name_emoji"] =               fit_sequence(test.name_emoji.str.lower(), tok_raw_emj)#, filt = False)
...
MAX_EMJ = max(tok_raw_emj.values())+1
...
def get_keras_data(dataset):
    X = {
        ...
        ,'emj': pad_sequences(dataset.seq_name_emoji, 
                              maxlen=max([len(l) for l in dataset.seq_name_emoji]))
...
def map_sort3(seq1, seq2, seq3):
	return sorted(range(len(seq1)), key=lambda x: len(seq1[x])*10000+len(seq2[x])*100+len(seq3[x]))
...
def reset_data(dt, bsize):
    ...
    sorted_ix = np.array(map_sort3(dt["seq_item_description"].tolist(), \
                                    dt["seq_name_token"].tolist(), \
                                    dt["seq_name_emoji"].tolist()))
...
def get_model():
    ...
    emb_item_condition      = Embedding(MAX_CONDITION, 4)(item_condition)
    emb_emj                 = Embedding(MAX_EMJ, 4)(emj) 
    ...
    rnn_layer5 = GRU(4, recurrent_dropout=0.0) (emb_emj)
    ...
    #model
    model = Model([brand, ntk, item_desc, emj #, name
...
lr_init, lr_fin = 0.015, 0.01
#lr_init, lr_fin = 0.02, 0.01
...
val_sorted_ix = np.array(map_sort3(dvalid["seq_item_description"].tolist(), 
                                   dvalid["seq_name_token"].tolist(), \
                                   dvalid["seq_name_emoji"].tolist()))
tst_sorted_ix = np.array(map_sort3(test  ["seq_item_description"].tolist(), 
                                   test  ["seq_name_token"].tolist(), \
                                   test  ["seq_name_emoji"].tolist()))
y_pred_epochs = []
yspred_epochs = []
for c, lr in enumerate([0.7, 0.9, 0.7]):
#for c, lr in enumerate([0.07, 0.09, 0.07]):
#for c, lr in enumerate([0.009, 0.008, 0.007]):
    ...
    yspred_epochs.append(model.predict_generator(
                    tst_generator(test.iloc[tst_sorted_ix], (batchSize*2))
                    , steps = int(np.ceil(test.shape[0]*1./(batchSize*2)))
                    , max_queue_size=1 
...
# Bagged Epoch 5 rmsle 0.431592501166
print('[{}] Finished NNET MODEL...'.format(time.time() - start_time))
...
llcols = [("seq_category_name_split", MAX_CAT), ("seq_item_description", MAX_DSC), \
          ("seq_name_token", MAX_NTK), ("seq_name_emoji", MAX_EMJ)]
#llcols = [("seq_category_name_split", MAX_CAT), ("seq_item_description", MAX_DSC), \
...
print(spmattst.shape)
print(spmattrn.shape)
print('[{}] Finished LGBM DATA...'.format(time.time() - start_time))
...
print('[{}] Finished LGBM MODEL...'.format(time.time() - start_time))

####[4000]  training's rmse: 0.423479       valid_1's rmse: 0.451184
####[7500]  training's rmse: 0.404565       valid_1's rmse: 0.446218
# [6061]  training's rmse: 0.387538       valid_1's rmse: 0.445166
...
print('[{}] Finished LGBM PREDICTION...'.format(time.time() - start_time))

# LGB trees 6061 rmsle 0.445165754192
...
submission.to_csv("../sub/myBag_2504.csv", index=False)
