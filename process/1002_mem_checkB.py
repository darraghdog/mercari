...
def reset_data(dt, bsize):
    ...
    if drop_cols:
        sorted_ix = np.array(map_sort(dt["seq_item_description_drop"].tolist(), dt["seq_name_token_drop"].tolist()))
    else:
...
def trn_generator(dn, dt, y, bsize):
    while True:
        ...
        for batch in batch_steps:
            ...
            Xbatch['dense_name'] = dn[ix_]#densetrn[ix_]#
...
def predictFMFTRL(mergetst):
    ...
    for i in range(0, len(cross_nm)):
        if i == 0:
            ...
            x_col = hstack((x_col, moddict['cross_cols'][cross_nm[i]].transform(mergetst[cross_nm[i]])))
...
def seqTokenDf(df):
    ...
    drop_set = set(tok_raw_cat_drop.keys())
    df['seq_category_name_split_drop']  = [[item for item in l if item not in drop_set] for l in df.seq_category_name_split]
    drop_set = set(tok_raw_dsc_drop.keys())
    df['seq_item_description_drop']     = [[item for item in l if item not in drop_set] for l in df.seq_item_description]
    df['seq_item_description_rev_drop'] = [[item for item in l if item not in drop_set] for l in df.seq_item_description_rev]
    drop_set = set(tok_raw_nam_drop.keys())
    df['seq_name_drop']                 = [[item for item in l if item not in drop_set] for l in df.seq_name ]
    drop_set = set(tok_raw_ntk_drop.keys())
    df['seq_name_token_drop']           = [[item for item in l if item not in drop_set] for l in df.seq_name_token]
    cpuStats()
...
tok_raw_cat_drop = dict([(v, k) for (k, v) in tok_raw_cat.items() if len(k)==1 or k in stoppers])
tok_raw_nam_drop = dict([(v, k) for (k, v) in tok_raw_nam.items() if len(k)==1 or k in stoppers])
tok_raw_dsc_drop = dict([(v, k) for (k, v) in tok_raw_dsc.items() if len(k)==1 or k in stoppers])
tok_raw_ntk_drop = dict([(v, k) for (k, v) in tok_raw_ntk.items() if len(k)==1 or k in stoppers])
...
def get_keras_data(dataset):
    if drop_cols:
        X = {
            'name': pad_sequences(dataset.seq_name_drop, 
                                  maxlen=max([len(l) for l in dataset.seq_name_drop]))
            ,'ntk': pad_sequences(dataset.seq_name_token_drop, 
                                  maxlen=max([len(l) for l in dataset.seq_name_token_drop]))
            ,'item_desc': pad_sequences(dataset.seq_item_description_drop, 
                                  maxlen=max(1, max([len(l) for l in dataset.seq_item_description_drop])))
            ,'item_desc_rev': pad_sequences(dataset.seq_item_description_rev_drop, 
                                  maxlen=max(1, max([len(l) for l in dataset.seq_item_description_rev_drop])))
            ,'brand': np.array(dataset.brand)
            ,'category_name_split': pad_sequences(dataset.seq_category_name_split_drop, 
                                  maxlen=max([len(l) for l in dataset.seq_category_name_split_drop]))
...
for b in range(bags):
    if b == 1:
        drop_cols = True        
        val_sorted_ix = np.array(map_sort(dvalid["seq_item_description_drop"].tolist(), dvalid["seq_name_token_drop"].tolist()))
    else: 
        drop_cols = False
    ...
    for i in range(epochs):
        model.fit_generator(
                            ...
                            , epochs=1
                            , max_queue_size=1
                            , steps_per_epoch = int(np.ceil(dtrain.shape[0]*1.*.8/batchSize))
    ...
    for c, lr in enumerate(lr_ls): # , 0.006, 0.007,
        ...
        model.fit_generator(
                            ...
                            , epochs=1#,epochs
                            , max_queue_size=1
                            , steps_per_epoch = int(np.ceil(dtrain.shape[0]*1.*0.75/batchSize))
                            , validation_data = val_generator(denseval, dvalid, dvalid.target, batchSize)
                            , validation_steps = int(np.ceil(dvalid.shape[0]*1./batchSize))
                            , verbose=2
                            )
...
def inference_keras(mergetst):
    ...
    for c in range(len(lr_ls)*bags):
        ...
        if c > 1:
            ...
            drop_cols = False
            tst_sorted_ix = np.array(map_sort(mergetst["seq_item_description_drop"].tolist(), mergetst["seq_name_token_drop"].tolist()))
