...
def getFMFTRL():
    ...
    merge['intersection'] = [list(set(n.lower().split()) & set(d.lower().split())) for (n, d) in zip(merge.name.values, merge.item_description.values)]
    merge['intersection'] = [' '.join([w for w in l if w not in stopw]) for l in merge['intersection']]
...
merge['brand'] = le.fit_transform(merge.brand_name).astype(np.int16)
merge['cat2'] = le.fit_transform(merge.subcat_2).astype(np.int16)
merge['cat1'] = le.fit_transform(merge.subcat_1).astype(np.int16)
merge['cat0'] = le.fit_transform(merge.general_cat).astype(np.int16)
del le
def dummy_small(series_):
    vcts_ = series_.value_counts()
    series_[series_.isin(vcts_[vcts_<200].index.values)] = (2**16-1)
    return series_
merge['cat2'] = dummy_small(merge['cat2'])
merge['cat1'] = dummy_small(merge['cat1'])
...
def posn_to_sparse(dt, embedding_map):
    ...
    embid_lens = []
    for c, (nm, ct) in enumerate(zip(dt['name_token'].values,
                               dt['category_token'].values)):
        sent = " ".join([nm, ct])
        ids = [embedding_map[s] for s in sent.split(' ') if (s in embw) and (s not in stopw)]
        n_ = len(ids)
        sprow += [c]*n_
        spcol += ids
        spdata += [1]*n_    
        if n_ == 0:
            embid_lens.append(0)
            continue
        embid_lens.append(1./n_)
        #pd.Series(embid_lens).hist()
    shape_ = (dt.shape[0], len(embedding_map.keys())) 
    dt_ids = csr_matrix((spdata, (sprow, spcol)), shape=shape_)
    return dt_ids, np.array(embid_lens)

# Make a sparse matrix of the ids of words
merge_ids, embid_lens = posn_to_sparse(merge, embedding_map)
# Get the dense layer input of the text
#densemrg = merge_ids.dot(embeddings_matrix)#.todense()
densemrg = np.multiply(merge_ids.dot(embeddings_matrix), np.expand_dims(embid_lens, 1))
...
MAX_CAT0 = np.max(merge.cat0.max())+1
MAX_CAT1 = np.max(merge.cat1.max())+1
