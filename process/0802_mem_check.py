...
def mergeEmbeddings(mergedf):
    ...
    densedf -= emb_mean_.astype('float16')
    densedf /= emb_sd_.astype('float16')
...
densetrn = mergeEmbeddings(dtrain) # densetrain[:nrow_train][trnidx] 
...
for b in range(bags):
    ...
    for c, lr in enumerate(lr_ls): # , 0.006, 0.007,
        ...
        model.fit_generator(
                            ...
                            , epochs=1#,epochs
                            , max_queue_size=1
                            , steps_per_epoch = int(np.ceil(dtrain.shape[0]*1.*0.5/batchSize))
                            , validation_data = val_generator(denseval, dvalid, dvalid.target, batchSize)
                            , validation_steps = int(np.ceil(dvalid.shape[0]*1./batchSize))
                            , verbose=2
                            )
...
# del mergetrn, densetrn, denseval
gc.collect()
cpuStats()
...
def trn_generator_new(dt, y, bsize):
    while True:
        max_step, batch_steps, sorted_ix, dt = reset_data(dt, bsize)
        for batch in batch_steps:
            from_ = batch*bsize
            ...
            ix_   = sorted_ix[from_:to_]
            Xbatch = dt.iloc[ix_]
            Xbatch = new_get_keras_data(Xbatch)
            ybatch = dt.target.iloc[ix_]
            yield Xbatch, ybatch

def val_generator_new(dt, y, bsize):
    while 1:
        max_step, batch_steps, sorted_ix, dt = reset_data(dt, bsize)
        for batch in batch_steps:
            from_ = batch*bsize
...
def tst_generator_new(dt, bsize):
    while 1:
        for batch in range(int(np.ceil(dt.shape[0]*1./bsize))):
        #for batch in range(dt.shape[0]/bsize+1):
            from_ = batch*bsize
...
def new_get_keras_data(dataset):
    X = {
        'name': pad_sequences(dataset.seq_name, 
                              maxlen=max([len(l) for l in dataset.seq_name]))
        ,'ntk': pad_sequences(dataset.seq_name_token, 
...
def new_rnn_model(lr=0.001, decay=0.0):
    ...
    emb_item_desc = Embedding(MAX_DSC, 60)(item_desc)
    emb_brand_name = Embedding(MAX_BRAND, 10)(brand)
    emb_category_name_split = Embedding(MAX_CAT, 10)(category_name_split) 
    emb_item_condition = Embedding(MAX_CONDITION, 5)(item_condition)
    ...
    model = Model([name, item_desc, brand, category_name_split, item_condition, num_vars], output)
...
rnn_model.fit_generator(
                    trn_generator_new(dtrain, dtrain.target, batchSize)
                    , epochs=epochs
                    , max_queue_size=1
                    , steps_per_epoch = int(np.ceil(dtrain.shape[0]*1.*0.9/batchSize))
                    , validation_data = val_generator_new(dvalid, dvalid.target, batchSize)
                    , validation_steps = int(np.ceil(dvalid.shape[0]*1./batchSize))
                    , verbose=1
                    )

K.set_value(rnn_model.optimizer.lr, 0.009)
rnn_model.fit_generator(
                    trn_generator_new(dtrain, dtrain.target, batchSize)
                    , epochs=2#,epochs
                    , max_queue_size=1
...
y_pred_epochs.append(rnn_model.predict_generator(
                tst_generator_new(dvalid.iloc[val_sorted_ix], batchSize)
                , steps = int(np.ceil(dvalid.shape[0]*1./batchSize))
                , max_queue_size=1 
                , verbose=2)[val_sorted_ix.argsort()])
...
submFM.to_csv("./FM0602.csv", index=False)
submRNN.to_csv("./RNN0602.csv", index=False)
