...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 19 19:39:59 2018
...
def getFMFTRL():
    ...
    mask = np.array(np.clip(sparse_merge[:nrow_train][trnidx].getnnz(axis=0) - 1, 0, 1), dtype=bool)
    ...
    model = FM_FTRL(alpha=0.01, beta=0.01, L1=0.00001, L2=0.1, D=sparse_merge.shape[1], 
...
y_pred_epochs = []
yspred_epochs = []
for c, lr in enumerate([0.010, 0.009, 0.008]): # 0.007,
...
y_pred = sum(y_pred_epochs[:-1])/len(y_pred_epochs[:-1])
yspred = sum(yspred_epochs[:-1])/len(yspred_epochs[:-1])
