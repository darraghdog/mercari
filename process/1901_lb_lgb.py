...
def main():
    ...
    tv = TfidfVectorizer(min_df=1, max_features=MAX_FEATURES_ITEM_DESCRIPTION,
            strip_accents='unicode', analyzer='word',token_pattern=r'\w{1,}',
            ngram_range=(1, 3), use_idf=1,smooth_idf=1,sublinear_tf=1,
            stop_words = 'english') 
    X_description = tv.fit_transform(merge['item_description']).astype('float16')
    #svd = TruncatedSVD(n_components=1000, n_iter=5, random_state=0, tol=0.0)
    #X_description = svd.fit_transform(X_description)    
    ...
    X[:,    ((X != 0).sum(0)>200).tolist()[0]]
    X.sum(axis=0)
    1481661*76408/ 63898102
    X[:10,:10].todense()
    '''
    lgb.Dataset()
    ...
    print('[{}] Predict lgb 1 started.'.format(time.time() - start_time))
    
    model = lgb.train(params3, train_set=d_train, num_boost_round=5000, valid_sets=watchlist, \
