...
tok_raw_brd = myTokenizerFit(train.brand_token.str.lower().unique(), max_words = 3000)
...
train["seq_brand_token"] =              fit_sequence(train.brand_token.str.lower(), tok_raw_brd, filt = False)
test["seq_brand_token"] =               fit_sequence(test.brand_token.str.lower(), tok_raw_brd, filt = False)
...
MAX_BRD = max(tok_raw_brd.values())+1
...
def get_keras_data(dataset):
    X = {
        ...
        ,'brd': pad_sequences(dataset.seq_brand_token, 
                              maxlen=max([len(l) for l in dataset.seq_brand_token]))
...
def get_model():
    ...
    emb_brd                 = Embedding(MAX_BRD, emb_size//2)(brd) 
    ...
    rnn_layer5 = GRU(8, recurrent_dropout=0.0) (emb_brd)
    ...
    #model
    model = Model([name, brand, ntk, item_desc, dense_name, brd
...
### with larger layers
# Bagged (3 times) Epoch 5 rmsle 0.429088545511
# Bagged (6 times) Epoch 5 rmsle 0.4241888317
...
pearsonr(y_pred, y_predlgb)
pearsonr(y_pred[:,0], dvalid.target.values)
pearsonr(y_predlgb[:,0], dvalid.target.values)
