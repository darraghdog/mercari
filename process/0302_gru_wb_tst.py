...
submFM = pd.concat([df[["test_id"]] for df in tstls]).astype(int)
submFM["price"] = np.expm1(predsFM)
submFM.to_csv("./submFM0404.csv", index=False)
submFM.head(20)
...
for col in ['name_token', 'category_token']:
    ...
    wordlist += [k for (k, v) in flat_counter.items() if v>1]
...
submRNN = mergetst[["test_id"]].astype(int)
submRNN["price"] = np.expm1(yspred[:,0])
submRNN.to_csv("./submRNN0404.csv", index=False)
submRNN.head(20)
