...
def getFMFTRL(moddict):
    ...
    moddict['wb_name_mask'] = np.array(np.clip(X_name[:nrow_train].getnnz(axis=0) - 1, 0, 1), dtype=bool)
    ...
    moddict['wb_cat_dict'] = dict([(c, X_cat_tmp.getrow(row)) for (c, row) in zip(cats.tolist(), range(len(cats)))])
    ...
    moddict['all_mask'] = np.array(np.clip(sparse_merge.getnnz(axis=0) - 1, 0, 1), dtype=bool)
...
merge, moddict, trnidx, validx, nrow_train, nrow_test, glove_file, predsFM, predsfm = getFMFTRL({})
cpuStats()   
gc.collect()
cpuStats()
