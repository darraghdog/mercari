...
tst_file = trn_file #= '../data/test.tsv'
cpuStats()
moddict = {}
mdir    = '../feat'
...
print("Bagged FM & Nnet", rmsle(mergetrn.price.values[:nrow_train][validx], np.expm1(predsfm)))
# ('Bagged FM & Nnet', 0.4122719249855641)
print("Bagged FM & Nnet", rmsle(mergetrn.price.values[:nrow_train][validx], pd.Series(np.expm1(predsFM[:nrow_train])[validx])))
...
merge_ids_name = posn_to_sparse_name(mergetrn, embedding_map).astype(np.int16)
merge_ids_cat = posn_to_sparse_cat(mergetrn, embedding_map).astype(np.int16)
densetrain  =  np.hstack((merge_ids_name.dot(embeddings_matrix)* 1./(merge_ids_name.sum(axis=1)+.5).astype('float16'), \
                          merge_ids_cat .dot(embeddings_matrix)* 1./(merge_ids_cat .sum(axis=1)+.5).astype('float16')))
# densetrain =  merge_ids.dot(embeddings_matrix)#.astype('float16')
glove_mean_, glove_sd_ = densetrain.mean(), densetrain.std()
glove_mean_, glove_sd_ 
densetrain -= glove_mean_
densetrain /= glove_sd_
print(mergetrn.shape)
print(densetrain.shape)
del merge_ids_name, merge_ids_cat
gc.collect()
...
#del merge, densemrg
dtrain, dvalid     = mergetrn[:nrow_train].iloc[trnidx], mergetrn[:nrow_train].iloc[validx]
densetrn, denseval = densetrain[:nrow_train][trnidx], densetrain[:nrow_train][validx]
gc.collect()
...
for c, lr in enumerate([0.010, 0.009, 0.008]): # , 0.006, 0.007,
    ...
    model.save_weights('%s/rnn_model%s.h5'%(mdir, c))
...
del mergetrn, densetrn, denseval, densetrain
gc.collect()
cpuStats()
'''
...
# Make a sparse matrix of the ids of words
merge_ids_name = posn_to_sparse_name(mergetst, embedding_map).astype(np.int16)
merge_ids_cat = posn_to_sparse_cat(mergetst, embedding_map).astype(np.int16)
densetest  =  np.hstack((merge_ids_name.dot(embeddings_matrix)* 1./(merge_ids_name.sum(axis=1)+.5).astype('float16'), \
                          merge_ids_cat .dot(embeddings_matrix)* 1./(merge_ids_cat .sum(axis=1)+.5).astype('float16')))
densetest -= glove_mean_
densetest /= glove_sd_
print(mergetst.shape)
print(densetest.shape)
del merge_ids_name, merge_ids_cat
gc.collect()
cpuStats()
yspred_epochs = []
...
print("Bagged FM & Nnet", rmsle(dvalid.price.values, pd.Series(np.expm1(predsFM[:nrow_train])[validx])))
print("Bagged FM & Nnet", rmsle(dvalid.price.values, pd.Series(np.expm1(yspred[:,0][:nrow_train])[validx])))
