...
def getFMFTRL():
    ...
    s = '24K GOLD plated rose'
    re.match(r"(?<!\d)\d{2}k(?!\d)", s.lower())
        
    '''
    Concat brand and name
    '''
    ...
    '''
    '''
    cols = ['brand_name', 'category_name', 'item_description']
    X_miss = np.zeros((merge.shape[0], len(cols)))
    for c, col in enumerate(['brand_name', 'category_name', 'item_description']):
        X_miss[:,c] = (merge[col] == merge[col]).values.astype(int)
    X_miss = coo_matrix(X_miss)
    '''
    ...
    '''
    Grep certain patterns
    '''
    '''
    gold = [re.findall(r"(?<!\d)\d{2}k(?!\d)", s.lower()) for s in col_val]
    merge['gold'] = [int(s[0][:2]) if s!=[] else 0 for s in gold]
    merge['gold'][~merge['gold'].isin([10,14,18,24])] = 0 
    lb = LabelBinarizer(sparse_output=True)
    X_gold = lb.fit_transform(merge['gold'])
    '''
    def extract_after_word(s,word):
        ...
        if word in s:
            ...
            if srch!=None:
                output=srch.group(0)
        return output
    # https://github.com/ChenglongChen/Kaggle_HomeDepot/blob/master/Code/Igor%26Kostia/feature_extraction1.py
    merge['name'].map(lambda x: extract_after_word(x,'for')).value_counts()
    merge['name'].map(lambda x: extract_after_word(x,'with')).value_counts()
    merge['name'].map(lambda x: extract_after_word(x,'without')).value_counts()
    merge['price'][merge['name'].map(lambda x: extract_after_word(x,'with'))=='tags'].mean()
    merge['price'][merge['name'].map(lambda x: extract_after_word(x,'with'))!='tags'].mean()
    
    str_common_word('hello', 'hello world')
    ...
    def stats():
        # String lengths
        nm_len  = merge['name'].str.len()
        ds_len  = merge['item_description'].str.len()
        nm_len = nm_len//10
        nm_len[nm_len>4] = 5
        ds_len1 = ds_len//100
        ds_len1[ds_len1>8] = 8
        ...
        ds_len2[nm_len>5] = 5
        # Number of words
        nm_words = merge['name'].str.split().apply(len)
        ds_words = merge['item_description'].str.split().apply(len)        
        nm_words1 = nm_words; nm_words1[nm_words1>10] = 10   
        ds_words1 = ds_words//2; ds_words1[ds_words1>10] = 20
        
        lb = LabelBinarizer(sparse_output=True)
        X_stats = hstack((lb.fit_transform(nm_len),
                          lb.fit_transform(ds_len1),
                          lb.fit_transform(ds_len2),
                          lb.fit_transform(nm_words1),
                          lb.fit_transform(ds_words1)))
        print ('Shape of stats hash', X_stats.shape)
        return X_stats
    X_stats = stats()
    ...
    for col in ['item_description', 'name']:    
        ...
        if 'X_orig' not in locals():
            sentcol = merge[col].copy()
            sentcol[(sentcol.str.len()>20)] = ' '
            X_orig = lb.fit_transform(sentcol.apply(hash))
            del sentcol
        else:
            sentcol = merge[col].copy()
            sentcol[(sentcol.str.len()>20)] = ' '
            X_orig = hstack((X_orig, lb.fit_transform(sentcol.apply(hash))))
            del sentcol
    #X_orig = hstack((X_orig, lb.fit_transform((merge['name']+merge['item_description']).apply(hash))))
    X_orig = X_orig.tocsr()
    X_orig = X_orig[:, np.array(np.clip(X_orig.getnnz(axis=0) - 2, 0, 1), dtype=bool)]
    X_orig = X_orig[:, np.array(np.clip(X_orig.getnnz(axis=0) - 1000, 1, 0), dtype=bool)]    
