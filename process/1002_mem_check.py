...
def trainFMFTRL(moddict):
    ...
    # model2
    train_X.shape
    moddict['train_X2'] = np.where(train_X.getnnz(axis=0) > 15)[0]
    train_X, valid_X  = train_X[:,moddict['train_X2']], valid_X[:,moddict['train_X2']]
    model2 = FM_FTRL(alpha=0.01, beta=0.01, L1=0.00001, L2=0.1, D=train_X.shape[1], alpha_fm=0.01, L2_fm=0.0, init_fm=0.01,
                    D_fm=300, e_noise=0.0001, iters=1, inv_link="identity", threads=4) #iters=15
    ...
    for i in range(10):
        #cpuStats()
        print (50*'-')
        model2.fit(train_X , train_y , verbose=1)
        #if i > 3:
        predsfm2 = model2.predict(X=valid_X)
        score_ = rmsle(np.expm1(valid_y), np.expm1(predsfm2))
        print("FM_FTRL dev RMSLE:", score_)
        #if score_ < baseline - 0.0004:
            #    baseline = score_
            #else:
    ...
    predsfmbag = (0.5*predsfm+0.5*predsfm2)
    print("FM_FTRL bagged dev RMSLE:", rmsle(np.expm1(valid_y), np.expm1(predsfmbag)))

    moddict['FMmodel'] = model
    moddict['FMmodel2'] = model2
    
    print('[{}] Train ridge v2 completed'.format(time.time() - start_time))
    gc.collect()
    
    return merge, moddict, trnidx, validx, nrow_train, predsfmbag, predsfm, predsfm2
...
def predictFMFTRL(mergetst):
    ...
    X_test = X_test[:,moddict['train_X2']]
    predsFM2 = moddict['FMmodel2'].predict(X_test)
    predsFMbag = (0.5*predsFM+0.5*predsFM2)
...
moddict = {}
_, _, _, _, _, predsfm, predsfm1, predsfm2 = trainFMFTRL({})
...
print("Bagged FM & Nnet", rmsle(dvalid.price.values, np.expm1(predsfm)*0.6 + np.expm1(y_pred[:,0])*0.4  ))
